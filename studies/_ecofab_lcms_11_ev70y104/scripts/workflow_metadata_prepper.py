...
# Input is _ecofab_lcms_11_ev70y104/mapped_files_to_biosamples.csv ; we will add and adjust columns as needed
# Raw data directory: /Volumes/LaCie/nmdc_data/_ecofab_11_ev70y104/raw
# Processed data directory: /Volumes/LaCie/nmdc_data/_ecofab_11_ev70y104/processed_20251022
import os
import pandas as pd
...
def add_processed_data_directory(mapped_df, processed_data_dir):
    ...
    # remove any trailing .raw (case-insensitive) before appending .corems
    mapped_df['processed_data_directory'] = (
        processed_data_dir
        + mapped_df['raw_data_file_short'].str.replace(r'(?i)\.raw$', '', regex=True)
        + '.corems'
    )
    return mapped_df
...
if __name__ == "__main__":
    final_dir = '_ecofab_lcms_11_ev70y104/metadata/metadata_gen_input_csvs/'
    # Make this dir, empty if it exists
    os.makedirs(final_dir, exist_ok=True)
    for f in os.listdir(final_dir):
        os.remove(os.path.join(final_dir, f))
    # Read in the mapped files to biosamples csv
    mapped_df = pd.read_csv('_ecofab_lcms_11_ev70y104/mapped_files_to_biosamples.csv')
    
    # The CSV already has most columns, but we need to extract raw_data_file_short for merging
    mapped_df['raw_data_file_short'] = mapped_df['raw_data_file'].str.split('/').str[-1]
    
    # The CSV already has instrument_analysis_end_date and instrument_instance_specifier columns
    # so we don't need to add them from the raw file info
    print("Using existing instrument times from CSV data.")
    
    # Update file paths to correct locations (.raw files)
    mapped_df['raw_data_file'] = mapped_df['raw_data_file'].str.replace(
        'Users/heal742/Library/CloudStorage/OneDrive-PNNL/Documents/_DMS_data/_NMDC/_massive/_ecofab_lcms/to_process',
        '/Volumes/LaCie/nmdc_data/_ecofab_11_ev70y104/raw/'
    )
    mapped_df['processed_data_directory'] = mapped_df['processed_data_directory'].str.replace(
        '/Users/heal742/Library/CloudStorage/OneDrive-PNNL/Documents/_DMS_data/_NMDC/_massive/_ecofab_lcms/processed_DATESTAMP',
        '/Volumes/LaCie/nmdc_data/_ecofab_11_ev70y104/processed_20251022'
    )
    
    # Add sample_id column (aliasing biosample_id)
